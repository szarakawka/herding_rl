...
# based on tensorforse.execution.Runner
class MultiClonesRoundRobinRunner(BaseRunner):
    """
    Simple runner for non-realtime single-process execution.
    """

    def __init__(self, agent, num_agent_clones, environment, momentum=0., repeat_actions=1, history=None, id_=0):
        """
        ...
        """
        super(MultiClonesRoundRobinRunner, self).__init__(agent, environment, repeat_actions, history)
        ...
        self.num_agent_clones = num_agent_clones
    ...
    def run(self, num_timesteps=None, num_episodes=None, max_episode_timesteps=None, deterministic=False,
        ...
        default_action = np.zeros(shape=self.environment.actions['shape'], dtype=np.float32)
        ...
        while True:
            ...
            while True:
                ...
                actions[which_clone] = self.agent.act(states=state[which_clone], deterministic=deterministic)

                reward = 0
                for repeat in range(self.repeat_actions):
                    state, terminal, step_reward = self.environment.execute(actions=actions)
                    reward += step_reward
                    if terminal:
                        break
                ...
                # apply momentum # TODO this logic should belong to agents and should be there, not here
                for i in range(len(actions)):
                    actions[i] = self.momentum * actions[i]
